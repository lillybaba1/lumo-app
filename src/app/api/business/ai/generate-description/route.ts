...
export async function POST(req: Request) {
  try {
    ...
    const isApproved = businessAccount.status === 'ACTIVE' ||
      (businessAccount.accountApproved && businessAccount.boutiqueApproved);
    
    if (!isApproved) {
      return NextResponse.json(
        { error: 'Business account must be approved to use AI features' },
        { status: 403 }
      );
    }
    ...
    console.log('[Seller AI Content] Generating:', {
      type,
      productName,
      category,
...
Return ONLY valid JSON array, nothing else.`;
    ...
    console.log('[Seller AI Content] Generated successfully:', type);
    ...
    if (type === 'attributes' || type === 'seo' || type === 'tags') {
      try {
        ...
        console.error(`[Seller AI Content] Failed to parse ${type} JSON:`, text);
    ...
    console.error('[Seller AI Content] Error:', {
